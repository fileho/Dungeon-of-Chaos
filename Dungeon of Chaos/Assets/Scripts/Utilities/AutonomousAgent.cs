...
namespace Helper
{
    public class AutonomousAgent
    {
        ...
        public static Vector3 PursueGetSteering(Rigidbody2D selfRB, Rigidbody2D target, float lookAheadDistance, float moveSpeed)
        {
        ...
        public static Vector3 SeparationGetSteering(Collider2D selfCollider, Collider2D[] targets, int targetsCount, float maxSepDist, float sepMaxAcceleration)
        {
            ...
            for (int i = 0; i < targetsCount; i++)
            {
                ...
                if (dist < maxSepDist)
                {
                    ...
                    var strength = sepMaxAcceleration * (maxSepDist - dist) / (maxSepDist - selfColliderSize - colliderSize);
        ...
        public static Vector3 EvadeGetSteering(Rigidbody2D selfRB, Rigidbody2D target, float maxPrediction, float maxAccel, float panicDist, bool decelerateOnStop, float timeToTarget)
        {
            /* Calculate the distance to the target */
            Vector3 displacement = target.position - selfRB.position;
            float distance = displacement.magnitude;
        ...
        public static Vector3 EvadeGetSteering(Rigidbody2D selfRB, Vector2 targetPos, float maxPrediction, float maxAccel, float panicDist, bool decelerateOnStop, float timeToTarget)
        {
            /* Calculate the distance to the target */
        ...
        public static Vector3 FleeGetSteering(Rigidbody2D selfRB, Vector3 targetPosition, float maxAcceleration, float panicDist, bool decelerateOnStop, float timeToTarget)
        {
            /* Get the direction */
