...
public static class Tweens
{
    ...
    public static float EaseOutElastic(float currentTime)
    {
        ...
        if (currentTime == 1)
            return 1;
        return (float)Math.Pow(2, -10 * currentTime) *
                   (float)Math.Sin((currentTime * 10 - 0.75) * EASE_OUT_ELASTIC_MULTIPLIER) +
               1;
    }

    public static float EaseOutBounce(float currentTime)
    {
        if (currentTime < 1f / EASE_OUT_BOUNCE_MULTIPLIER_D)
            return EASE_OUT_BOUNCE_MULTIPLIER_N * currentTime * currentTime;
        if (currentTime < 2f / EASE_OUT_BOUNCE_MULTIPLIER_D)
            return EASE_OUT_BOUNCE_MULTIPLIER_N * (currentTime -= 1.5f / EASE_OUT_BOUNCE_MULTIPLIER_D) * currentTime +
                   0.75f;
        if (currentTime < 2.5f / EASE_OUT_BOUNCE_MULTIPLIER_D)
            return EASE_OUT_BOUNCE_MULTIPLIER_N * (currentTime -= 2.25f / EASE_OUT_BOUNCE_MULTIPLIER_D) * currentTime +
                   0.9375f;
        return EASE_OUT_BOUNCE_MULTIPLIER_N * (currentTime -= 2.625f / EASE_OUT_BOUNCE_MULTIPLIER_D) * currentTime +
               0.984375f;
    }
    ...
    public static float EaseInExponential(float currentTime)
    {
        if (currentTime == 0)
            return 0;
        return (float)Math.Pow(2, 10 * (currentTime - 1));
    }
    ...
    public static float EaseOutExponential(float currentTime)
    {
        if (currentTime == 1)
            return 1;
        return (1 - (float)Math.Pow(2, -10 * currentTime));
    }
